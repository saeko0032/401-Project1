...
struct Account createAnAccount(char* studentIdi, char*password);
    ...
    for(int i=0; i<10; i++)
    {
        ...
        for(int i=0; i<numberOfAccounts; i++)
        {
            if((strcmp(userName,listOfAccounts[i].studentId) == 0 ) &&
               (strcmp(password,listOfAccounts[i].password) == 0 ))
...
struct Student getMyStudentDataById(char* studentId)
{
    ...
    for(int i=0; i<10; i++)
    {
        ...
        {
            if(strcmp(studentId, listOfStudents[i].name) == 0)
            {
...
struct Account* getListofAccountFromFile(char* fileAddress, char* fileName, int* numberOfAccounts)
{
    ...
    for(int k=0; k<numberOfRecords; k++)
    {
        ...
        listOfAccounts[numberOfStudentsReadFromFileSoFar] = account;
